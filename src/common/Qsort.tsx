...
export default function QuickSortVisualizer() {
  ...
  const handleStart = async () => {
    if (isSorting) return
    if (!arr || arr.length === 0) return alert("آرایه خالی است — ابتدا آرایه را وارد یا تولید کن.")
    setComparisons(0)
    setSwaps(0)
    setIsSorting(true)
    const copy = [...arr]
  ...
  return (
    <Card className="max-w-4xl mx-auto my-6 p-4">
      <CardHeader>
        <CardTitle className="text-lg">مرتب‌سازی سریع — Quick Sort (نمایش گرافیکی)</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex flex-wrap gap-3 mb-4">
          <Button onClick={handleRandom} disabled={isSorting}>تولید آرایه تصادفی</Button>
          <Button onClick={handleStart} disabled={isSorting}>شروع مرتب‌سازی</Button>
          <Button variant="outline" onClick={() => setStepMode(!stepMode)} disabled={isSorting}>
            {stepMode ? "غیرفعال کردن حالت مرحله‌ای" : "نمایش مرحله‌ای (گام‌به‌گام)"}
          </Button>
        ...
        <div className="flex gap-4 flex-wrap">
          ...
          <div className="flex flex-col gap-2 text-sm">
            ...
            <Button variant="outline" onClick={() => { setArr([...arr]); setComparisons(0); setSwaps(0) }} disabled={isSorting}>ریست</Button>
          </div>
        </div>
